...
export function UserTaskItem({ task }: UserTaskItemProps) {
    ...
    const projectName = task.project_name || task.team_name;
        ...
        >
            ...
            <span className="min-w-0 flex-1">
                ...
                <span className="mt-1 flex min-w-0 items-center gap-1.5 text-[9px] text-slate-400 sm:text-[10px]">
                    <span className={`size-1.5 shrink-0 rounded-full ${task.project_color ? '' : task.project_name ? 'bg-violet-400' : 'bg-sky-400'}`} style={task.project_color ? { backgroundColor: task.project_color } : undefined} />
